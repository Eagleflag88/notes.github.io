...
Group Convolution（分组卷积）是一种卷积神经网络中的操作，它将输入特征图（feature maps）分成多个组，并在每个组内单独进行卷积操作。这种方法由Alex Krizhevsky在AlexNet中首次引入，主要是为了在GPU上并行化计算，以处理计算和存储资源有限的问题。后来，这种方法因其可以减少参数数量和计算量，同时增加网络的表示能力而流行开来。
...
如果输入特征图有C个通道，分为G组，那么每组有C/G个通道。对于每组，你会应用K个卷积核，这样总共就有G_K个卷积核。最终输出的特征图也会有G_K个通道，因为每组卷积产生K个通道的输出。
...
- 降低计算量：分组卷积减少了参数数量和计算量，因为每个卷积核只与部分输入通道进行卷积。
- 并行化：不同组可以在不同的计算单元上并行处理，提高了计算效率。
- 提高表征能力：它强迫网络在较小的通道组内学习特征，可能增加了特征的多样性。

## 缺点

- 减少特征融合：由于卷积核不跨越所有通道，特征图之间的交互减少，可能会影响学习到的特征的质量。
