...
- 实现：
	...
	2. Padding的机制在所有的Q，K，和V上都会实施。在计算注意力分数后，我们在 softmax 函数应用之前对这些被标记为 padding 的位置施加极小值（例如，将这些位置的分数设置为负无限大）；
...
- 实际操作时，只会用一套$W_Q$,$W_K$和$W_V$，然后把生成的Q，K和V切成几个部分，进行后面的处理。
