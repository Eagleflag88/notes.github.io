...
$$
\boldsymbol{R} = \boldsymbol{I} + sin(\theta)n^\wedge + (1 - cos(\theta))nn^{T}
$$

# 欧拉角

## Concept
- 欧拉角是描述刚体在三维空间中取向的一组三个角度。欧拉角通过三次旋转将一个**坐标系**转换到一个**目标坐标系**，这三次旋转通常围绕不同的轴进行；
- 欧拉角的三个角通常分别称为：
...
- XYZ顺序：先滚转，然后俯仰，最后偏航，这在机器人学中比较常见。此时，跟旋转矩阵的关系如下：
$$
R = R_z(\gamma)R_y(\beta)R_x(\alpha)
$$

## 右手系
- xyz本身的坐标是通过右手系定义的，特点是：
	- X轴指向右
	- Y轴指向上
	- Z轴指向前（或从观察者的视角来看，指向屏幕外）
- 在使用右手坐标系描述旋转时，通常遵循右手法则来确定旋转的正方向。右手法则规定，如果右手的拇指指向旋转轴的正方向，那么四指弯曲的方向就是正向旋转的方向；
...
$$
- 由此看出四元数只表示了一半的旋转；
- 因为三角函数的周期为$2\pi$，所以$q$和$-q$表示同一个旋转；
- Identity Quaternion：实部为1，虚部为0，表示$\theta=0$，即没有旋转；
- 四元数是最小的过参数化定义；
...
- 四元数的指数映射是一个数学操作，它将李代数（特别是四元数的全虚部分）中的元素映射到李群（在这种情况下，对应一个单位四元数）上；
...
- 如果四元数代表姿态，那么姿态的变化率$\dot{q}$和角速度$\Omega$的关系可以用下式表示：
$$
\dot{q} = \frac{1}{2}q \otimes \Omega
$$

- 其中$\otimes$是四元数乘法。$\Omega$是角速度的四元数形式，$\Omega=[0, \omega_x, \omega_y, \omega_z]$
