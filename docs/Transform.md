...
- 齐次坐标（Homogeneous Coordinates）是一种数学表示方法，它在计算机图形学、计算机视觉和机器人学等领域中非常有用。这种表示方法通过添加一个额外的维度来扩展传统的坐标系统，使得包括平移在内的仿射变换可以通过矩阵乘法来实现；
...
2. 统一变换表示： 齐次坐标使得所有的仿射变换（包括平移）都可以用矩阵乘法表示。这在计算机图形学中非常重要，因为可以将多个变换组合成一个矩阵，然后用这个矩阵一次性变换所有的点；

## 应用 

- 平移：在传统坐标中，平移需要加法。而在齐次坐标中，平移可以通过乘以一个包含平移向量的矩阵来实现；
- 旋转：旋转可以通过乘以一个旋转矩阵来实现，这和非齐次坐标系中的操作相同；
- 缩放：缩放通过乘以一个缩放矩阵来实现；    
- 透视投影：透视变换可以通过齐次坐标表示，这在三维图形的投影到二维屏幕上时非常有用；
...
2. $n$是与旋转轴一致的3维单位向量，DoF = 2。$n$是旋转矩阵$R$在特征值为1时的特征向量，在旋转轴上的向量经过旋转之后不会发生变化，满足$Rn = n$
3. 实际上就是旋转矩阵的李代数，可以用罗德里格斯公式转换为旋转矩阵；

# 欧拉角

## Concept
- 欧拉角是描述刚体在三维空间中取向的一组三个角度，通常用来表示飞行器、船舶、机器人臂等物体的方位。欧拉角通过三次旋转将一个参考坐标系转换到一个目标坐标系，这三次旋转通常围绕不同的轴进行。
- 欧拉角的三个角通常分别称为：
...
- 给定一个全虚四元数$u=0+xi+yj+zk$其中$x,y,z$ 表示绕一个特定轴的旋转向量，四元数的指数映射可以定义为：
$$
e^u=cos(||u||)+\frac{u}{||u||}sin(||u||)
$$
- 其中$||u||$是四元数的模，它表示旋转的角度；
- 结果是一个单位四元数，表示了对应于旋转向量$u$的有限旋转。这个单位四元数可以用来直接在三维空间中应用旋转；
