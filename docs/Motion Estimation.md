...
$\underline{运动恢复}$
- 对E进行SVD分解
- 可获得4个组合的t和R，得到的t只能up to a scale；
- 根据空间点在相机深度的positivity来筛选t和R的组合
...
- 已知n对3D-2D匹配点
	...
	- 以下推导为假设内参K已知
- 对每一个点对，$P_w$, $P_c$, $s$: $s*P_c = K*P*P_w$
- P是形式为[R|t]的3x4的投影矩阵
	1. 注意不是变换矩阵
...
- Process
	- 求解投影矩阵：
		...
		3. 利用SVD分解A求解获得X，X为V的最后一列；
	- 恢复R和t
		1. X = s[R, t]，X是有尺度的；
		2. 对X进行SVD分解：$X = UDV^T$
		3. 通过空间点应该在摄像机前获得R和s的符号
